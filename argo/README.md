...
**Secrets:** no plaintext committed. `svghunt0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret svghunt0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `svghunt0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `svghunt0` | `sdk-apps-dev` | svghunt0-dev-topia.topia-rtsdk.com | `/api/system/health` |
